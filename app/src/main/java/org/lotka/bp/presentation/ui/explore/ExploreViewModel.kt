...
constructor(
    ...
    val selectedCategory: MutableState<ExploreCategory?> = mutableStateOf(null)
    ...
    init {
        ...
        savedStateHandle.get<ExploreCategory>(STATE_KEY_SELECTED_CATEGORY)?.let { c ->
            setSelectedCategory(c)
        }

        if(recipeListScrollPosition != 0){
            onTriggerEvent(ExploreEvent.RestoreStateEvent)
        }
        else{
            onTriggerEvent(ExploreEvent.NewSearchEvent)
        }
    ...
    fun onTriggerEvent(event: ExploreEvent){
        viewModelScope.launch {
            try {
                when(event){
                    is ExploreEvent.NewSearchEvent -> {
                        newSearch()
                    }
                    is ExploreEvent.NextPageEvent -> {
                        nextPage()
                    }
                    is ExploreEvent.RestoreStateEvent -> {
                        restoreState()
                    }
                }
            }catch (e: Exception){
    ...
    fun onSelectedCategoryChanged(category: String) {
        val newCategory = getExploreCategory(category)
    ...
    private fun setSelectedCategory(category: ExploreCategory?){
